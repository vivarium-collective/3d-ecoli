"""Unit tests for ecoli_3d.composites.ecoli_structural."""
...
def test_baseline_parsimony_function_is_registered():
    ...
    from ecoli_3d.composites import ecoli_structural  # noqa: F401 — fires decorator
...
def test_pack_out_dir_reads_workspace_yaml_layout(tmp_path, monkeypatch):
    ...
    from ecoli_3d.composites.ecoli_structural import _resolve_pack_out_dir
...
def test_pack_out_dir_flat_default_when_layout_studies_unset(tmp_path, monkeypatch):
    ...
    resolver's conventional FLAT ``studies/`` layout (absolute). (the real
...
def test_generator_appends_pack_step_with_core_none():
    ...
    if not _REGISTRY:
        discover_generators()
    entry = _REGISTRY["ecoli_3d.composites.ecoli_structural.ecoli_structural"]
...
def _register_pack_step_hook_registers_link():
    from v2ecoli.core import build_core
    from ecoli_3d.composites.ecoli_structural import _register_ecoli_pack_step
    core = build_core()
