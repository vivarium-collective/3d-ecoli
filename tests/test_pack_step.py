...
from ecoli_3d import pack_step as ps
...
def _step(monkeypatch, snapshots, calls):
    ...
    monkeypatch.setattr(ps, "pack_from_state",
        lambda out_dir, name, counts, volume_fl, **k: calls.append((name, volume_fl)) or {"n_placed": 1})
...
def test_n_placed_reflects_pack_from_state_result(monkeypatch):
    """build_pack's real return dict has no 'placements' key, only
    'n_placed'; pack_status must reflect that field directly."""
    calls = []
    step = ps.EcoliPackStep
    monkeypatch.setattr(ps, "pack_from_state",
        lambda out_dir, name, counts, volume_fl, **k: calls.append((name, volume_fl)) or {"n_placed": 7})
    monkeypatch.setattr(ps, "bulk_to_counts", lambda bulk: {"X": 1})
    monkeypatch.setattr(ps, "bulk_to_locations", lambda bulk: {})
    monkeypatch.setattr(ps, "chromosome_state_from_live", lambda fc, rep=None: (1, 0.0))
    monkeypatch.setattr(ps, "rnaps_from_live", lambda *a, **k: [])
    inst = ps.EcoliPackStep(config={"snapshots": {"initial": 10.0}, "study": "s",
                                     "out_dir": "/tmp/o", "epsilon_s": 1.0})
    result = inst.update(_state(10.0))
    assert result["pack_status"]["initial"] == 7.0
