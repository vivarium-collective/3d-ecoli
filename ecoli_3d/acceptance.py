...
* :func:`check_selection_conservation` — the REPRODUCIBLE-NOW core: ecoli_3d's
  own counts -> ingredient selection path conserves per-species counts exactly
  and respects the top-N cap. Runs with only the installed deps (no parsimony
...
def check_selection_conservation(counts: dict[str, int], *, top_n: int = 40) -> dict[str, Any]:
    """Reproducible-now check: ecoli_3d's counts -> ingredient selection conserves
    ...
    try:
        ings = B.select_ingredients(counts, top_n=top_n)
        ings2 = B.select_ingredients(counts, top_n=top_n)
    finally:
        B.Ingredient = orig
